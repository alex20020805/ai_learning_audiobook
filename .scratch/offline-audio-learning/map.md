...
- [Prototype hierarchy-aware Episode packing](./issues/05-prototype-hierarchy-aware-episode-packing.md) — validate global hierarchy-aware packing under the default 15–25-minute policy, favor semantic and chapter boundaries, cross chapters to prevent short tails, and surface every unverified split for review.
- [Decide the local-first product workflow](./issues/06-decide-the-local-first-product-workflow.md) — center the Mac application on persistent hash-keyed Book workspaces, provisional plans, confirmed source spans, independent checkpointed Episode jobs, gated warnings and paid approval, complete local results, M4A delivery through iCloud Drive, configurable 5–30-minute bounds, and horizon-specific progress behavior.
